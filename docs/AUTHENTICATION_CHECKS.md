...
2. Keep every pet’s images under `resources/<pet-id>/`; ownership and
   visibility come from its Firestore document rather than its Storage path.
...
| Private upload | Images are written under `resources/<pet-id>/`; object metadata and the Firestore document carry the same `ownerUid`, with `isPublic: false`. |
